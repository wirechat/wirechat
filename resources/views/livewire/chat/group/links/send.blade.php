<div class="h-[calc(100vh_-_10rem)] sm:h-[450px] bg-[var(--wc-light-primary)] dark:bg-[var(--wc-dark-primary)] dark:text-white border border-[var(--wc-light-secondary)] dark:border-[var(--wc-dark-secondary)] overflow-y-auto overflow-x-hidden rounded-xl">
    <header class="sticky top-0 bg-[var(--wc-light-primary)] dark:bg-[var(--wc-dark-primary)] z-10 p-2">
        <div class="flex items-center pb-2">
            ...
            <h3 class="text-sm mx-auto font-semibold">{{ __('wirechat::chat.group.invite_link.send_via_chat.heading.label') }}</h3>
            ...
            <button wire:click="save" wire:loading.attr="disabled" wire:target="save"
                ...
                {{ __('wirechat::chat.group.invite_link.send_via_chat.actions.send.label') }}
            </button>
        </div>
                        ...
                        <li wire:key="invite-users-{{ md5((string) $user['type']) }}-{{ $user['id'] }}" class="flex cursor-pointer group gap-2 items-center p-2">
                            <label wire:click="toggleMember(@js((string) $user['id']), @js($user['type']))" class="flex cursor-pointer gap-2 items-center w-full min-w-0">
                                ...
                                <div class="ml-auto shrink-0">
                                    ...
                                    @else
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-plus-square-dotted w-6 h-6" viewBox="0 0 16 16">
