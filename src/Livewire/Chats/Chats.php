...
class Chats extends Component
{
    ...
    public function conversations()
    {
        ...
        $user = $this->auth;
        $ids = array_map('strval', $this->conversationIds);
        ...
        $conversations = Conversation::query()
            ...
            ->sortBy(fn (Conversation $c) => $positions[(string) $c->getKey()] ?? PHP_INT_MAX)
            ->values();
    ...
    protected function loadConversationIds(): void
    {
        ...
        $newIds = $rows->pluck('id')->map(fn ($id) => (string) $id)->all();
