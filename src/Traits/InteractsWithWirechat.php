...
trait InteractsWithWirechat
{
    ...
    public function createConversationWith(Model $peer, ?string $message = null): ?Conversation
    {
        ...
        $existing = WirechatService::conversationModelClass()::withoutGlobalScopes()
            ->where('type', $type)
        ...
        return DB::transaction(function () use ($type, $isSelf, $authType, $authId, $peerType, $peerId, $message) {
            ...
            $authParticipant = WirechatService::participantModelClass()::firstOrCreate(
                [
            ...
            if (! $isSelf) {
                WirechatService::participantModelClass()::firstOrCreate(
                    [
                        'conversation_id' => $conversation->getKey(),
